...
const ANCHOR = new Date("2026-07-30T00:00:00Z"); // fecha de referencia: día 1 = lanzamiento
...
function renderReminder(today){
  ...
  if(entry && (entry.rating || entry.listened === false)){
...
function showSurprise(){
  const today = chileDateStr();
  const todayAlbum = resolveAlbum(today);
  let pick;
  ...
  document.getElementById("surpriseBandcamp").href = bandcampSearchUrl(pick);
  document.getElementById("surpriseYoutube").href = youtubeSearchUrl(pick);
